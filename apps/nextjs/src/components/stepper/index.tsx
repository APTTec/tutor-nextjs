      ...
      <StepperProvider
        ...
        <div
          ...
          className={cn(
            ...
            variant === 'line' && orientation === 'horizontal' && 'gap-0',
            className,
            styles?.['main-container']
          )}
          style={
            {
              '--step-icon-size': variables?.['--step-icon-size'] ?? `${VARIABLE_SIZES[size]}`,
...
const VerticalContent = ({ children }: { children: React.ReactNode }) => {
  ...
  return React.Children.map(children, (child, i) => {
    const isCompletedStep =
      (React.isValidElement(child) && (child.props as StepProps).isCompletedStep) ?? i < activeStep
...
const HorizontalContent = ({ children }: { children: React.ReactNode }) => {
  ...
  return React.Children.map(childArr[activeStep], node => {
    ...
    return React.Children.map((node.props as StepProps).children, childNode => childNode)
  })
}
